...
contract AspisLiquidityCalculator is ITokenValueCalculator {
    ...
    function convert(address _token, uint256 _amount) public view returns (uint256) {
        ...
        return (_amount * uint256(_price))/(10 ** (decimals + _decimals));
    }
    ...
    function calculateTotalLiquidity(address _account) public view returns (uint256) {
        uint256 sum = 0;
        for(uint64 i = 0; i < _tokens.length; i++) {
            sum += convert(_tokens[i],ERC20(_tokens[i]).balanceOf(_account));
        }
        return sum;
    }
    ...
    function calculateFee(uint256 fee, uint256 value) public pure returns (uint256) {
        // TODO: Add SafeMath
